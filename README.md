...
- `scripts/`：打包阶段脚本，例如清理未使用 Electron locale 的 `after-pack.cjs`。
- `tests/`：关键工具函数测试。
- `docs/`：测试、打包和发布记录。
...
- 2026-06-21：第十阶段打包时将运行时不需要的前端库移入开发依赖，避免 `node_modules` 重复进入 `app.asar`；打包后仅保留 `zh-CN` 与 `en-US` locale。最终安装包约 89.27 MB，便携版约 89.05 MB，`win-unpacked/` 约 309.47 MB，`app.asar` 约 1.48 MB。
